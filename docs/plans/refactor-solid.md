...
- [ ] **R.C.1** Define `NowPlayingState`, `TransportCommands`, `QueueCommands`, `ReplayGainCommands` interfaces in `playback/`. `MiniPlayer` and `NowPlayingScreen` migrate to take only what they read (Playback-F4).
...
- [ ] **R.C.6** `PlaybackUiController` shrinks to the connection lifecycle + listener attach + composing the four collaborators. Target: under 200 LOC.
...
- [ ] **R.D.4** Hoist multi-select state out of the tab body into `rememberSelectionState()` — pure transition methods, unit-testable (UI-F13).
- [ ] **R.D.5** Move `firstDifference` / `clampMoveAwayFromActive` / `translateVisualToReal` from `QueueSection.kt` to `playing/QueueReorderLogic.kt` — pure logic, file-scope test seam unchanged (UI-F12).
