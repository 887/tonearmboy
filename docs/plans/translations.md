...
- [x] **T.A.3** Library tabs + detail screens: section headers, empty states, sort sheet, multi-select bar, filter chips, custom-tab editor. ~100 strings. *(shipped on worktree branch `translations/t-a-extract`. Extracted 123 strings + 9 plurals into `values/strings_library.xml`, covering the rail / top-bar / detail screens / sort sheet / filter sheet / custom-tab editor / `ConditionUi` registry / multi-select bar / track context menu / scan-progress bar / delete-file flow / playlist-detail / track-picker. `TabSpec.toTile` now takes a `Resources` so non-Composable subtitle copy ("X albums · Y tracks") can plural-translate. `ConditionUi.label` / `addSubtitle` flipped to `@StringRes` ids; `summary()` now takes a `Context`. `tabLabel` replaced with `tabLabelRes`. PlaylistPickerSheet / PlaylistsTilesScreen / PlaylistDialogHost left to T.A.6 per scope.)*
