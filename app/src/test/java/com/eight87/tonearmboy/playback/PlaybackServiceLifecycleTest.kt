 ...
 * tonearmboy` + the post-force-stop notification removal assertion).
 *
...
class PlaybackServiceLifecycleTest {
  ...
  private fun playbackServiceInfo(): ServiceInfo {
    ...
    return services.first { it.name == "com.eight87.tonearmboy.playback.PlaybackService" }
  }
  ...
  fun playback_service_responds_to_media_session_service_intent_filter() {
    ...
    val match = resolved.firstOrNull {
      it.serviceInfo.name == "com.eight87.tonearmboy.playback.PlaybackService"
    }
    assertNotNull(
  ...
  fun playback_service_component_resolves() {
    val cn = ComponentName(pkg, "com.eight87.tonearmboy.playback.PlaybackService")
    val info = pm.getServiceInfo(cn, 0)
    assertEquals("com.eight87.tonearmboy.playback.PlaybackService", info.name)
  ...
  fun task_removal_path_is_named_and_documented() {
    ...
    val cls = Class.forName("com.eight87.tonearmboy.playback.PlaybackService")
