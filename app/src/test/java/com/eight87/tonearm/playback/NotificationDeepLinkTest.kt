 ...
 *      `MainActivity` carrying the `tonearm.deeplink = "now_playing"`
 ...
 *      deeplink nonce, which causes `TonearmApp` to push
...
class NotificationDeepLinkTest {
  ...
  fun deeplink_constants_are_stable() {
    ...
    assertEquals("tonearm.deeplink", PlaybackService.EXTRA_DEEPLINK)
