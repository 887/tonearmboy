 ...
 *  - a `tonearm-albumart://<id>` opaque scheme pointing at a
...
class PlaylistCoverResolutionTest {
  ...
  @Test fun tonearm_albumart_scheme_resolves_to_album_art_branch() {
    val resolved = resolvePlaylistCover(
  ...
  @Test fun tonearm_albumart_with_unparseable_id_falls_through_to_first_track() {
    val resolved = resolvePlaylistCover(
      coverUri = "tonearm-albumart://not-a-number",
      firstTrackAlbumId = 1234L,
    )
    assertTrue(resolved is PlaylistCoverSource.AlbumArt)
