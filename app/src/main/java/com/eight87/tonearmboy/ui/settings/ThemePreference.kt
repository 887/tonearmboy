 ...
 * `Context.tonearmboyDataStore` extension below, so multiple constructions
...
class ThemePreferenceStore(private val context: Context) {
  ...
  val flow: Flow<ThemePreference> =
    context.tonearmboyDataStore.data.map { prefs ->
      ThemePreference.fromStored(prefs[KEY_THEME_MODE])
    }

  suspend fun set(value: ThemePreference) {
    context.tonearmboyDataStore.edit { prefs -> prefs[KEY_THEME_MODE] = value.name }
  }
...
internal val Context.tonearmboyDataStore: DataStore<Preferences> by preferencesDataStore(name = "tonearmboy_settings")
